...
const CFDPasswordModal = observer(({ form_error, platform }: TCFDPasswordModalProps) => {
    ...
    const success_modal_submit_label = getSuccessModalSubmitLabel({
    ...
    const getWalletHeader = (type: TType) => {
        return (
            <React.Fragment>
    ...
    const cfd_details = {
        app_icon: getWalletCFDIcon(account_type.type),
        ...
        <React.Fragment>
            ...
            {is_wallet_enabled && account_type.category === 'demo' && platform === CFD_PLATFORMS.MT5 ? (
                <WalletCFDSuccessDialog
                    header={getWalletHeader(account_type.type)}
